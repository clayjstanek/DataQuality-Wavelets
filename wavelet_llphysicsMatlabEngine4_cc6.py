...
tp = r'C:\users\cstan\documents\cooperstandard\tutorials\wavelets\Afl_BT120_run5_2023-01-15--041341-UTC1.datapkg'
...
COLS = df.shape[1]
T = np.arange(0, N) * dt
logging.debug(f"dt = {dt}, Num Cols = {COLS}")
...
p = Path(__file__)
p = p.parents[0]
ts_path = p.joinpath('dqs','timeseriespics090323')
if not ts_path.exists():
    Path.mkdir(ts_path, parents=True)
coherencepics_path = p.joinpath('dqs','coherencepics090323')
if not coherencepics_path.exists():
    Path.mkdir(coherencepics_path, parents=True)
coherenceraw_path = p.joinpath('dqs','coherenceraw090323')
...
for col_name1, col_ldx in tqdm(zip(Columns[:], range(COLS))):
    for col_name2, col_idx in tqdm(zip(Columns[col_ldx:], range(col_ldx, COLS))):
        ...
        for batch_idx in tqdm(range(BATCHES)):
            ...
            filename_pkl = coherenceraw_path.joinpath( f"wavecoherence_data[{col_ldx}][{col_idx}]batch[{batch_idx}].pkl")
            # Plot via MATLAB
...
#                matlab.double(16),
            ...
            # Saving the data in raw form
            with open(filename_pkl, 'wb') as f:
                pickle.dump([wavecoherencepy,wavephase], f)
            ...
            filename = coherencepics_path.joinpath(f"coherence_pics[{col_ldx}][{col_idx}]batch[{batch_idx}].png")
            plt.savefig(filename, format="png")
