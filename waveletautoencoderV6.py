...
logging.basicConfig(
    ...
    filename= p.joinpath('logs', 'waveletautoencoderV121323.log'),
...
for i,file_path in tqdm(enumerate(pickle_files_list)):
...
#image size = 55,64
"""
x = Conv2DTranspose(256, (3, 3), strides=(1, 2), padding='same')(decoded)
x = BatchNormalization()(x)
x = Activation('relu')(x)
...
x = Conv2DTranspose(128, (3, 3), strides=(1, 4), padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
...
x = Conv2DTranspose(64, (3, 3), strides=(1, 2), padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
...
#image size is 55,1024
x = Conv2DTranspose(32, (3, 3), strides=(2, 4),  padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
...
plot_model(autoencoder, to_file=p.joinpath('Autoencoder2DBatchNorm_wavelet_coherenceWin11.png'),
...
checkpoint = ModelCheckpoint(
    filepath= p.joinpath('autoencoder2DBatchNorm_checkpointWin11.h5'),  # Specify the file path where the model will be saved
...
autoencoder.save(p.joinpath('autoencoder2DBatchNorm_waveletcoherence121323final.h5'))
sys.exit(0)
...
labelspath = p.joinpath('test_labels_y')
with open(labelspath, 'wb') as file:
    pickle.dump((test_labels_array, test_labels), file)
